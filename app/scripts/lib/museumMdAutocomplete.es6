...
angular
    ...
    .controller('museumMdAutocompleteCtrl', museumMdAutocompleteCtrl);
...
function museumMdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  function selectedItemChange (selectedItem, previousSelectedItem) {
    if (selectedItem) {
      getDisplayValue(selectedItem).then(function (val) {
        ...
        // $scope.searchText = `${$scope.searchText} ${val}`;
  ...
  function select (index) {
    ...
    $mdUtil.nextTick(function () {
      getDisplayValue(ctrl.matches[ index ]).then(function (val) {
        ...
        let existing_vals = ngModel.$viewValue.split(/[ ,]+/g);
        let lastval = existing_vals[existing_vals.length - 1];

        let matchesany = ctrl.matches.map( (x) => x.display ).indexOf(lastval) !== -1;
        ...
        existing_vals.push(val);

        ngModel.$setViewValue(existing_vals.join(' '));
        ngModel.$render();
      }).finally(function () {
...
museumMdAutocompleteCtrl.$inject = ["$scope", "$element", "$mdUtil", "$mdConstant", "$mdTheming", "$window", "$animate", "$rootElement", "$attrs", "$q"];

angular
    .module('material.components.autocomplete')
    .directive('museumMdAutocomplete', museumMdAutocomplete);

/**
 * @ngdoc directive
 * @name museummdAutocomplete
 * @module material.components.autocomplete
 *
 * @description
...
function museumMdAutocomplete () {

  return {
    controller:   'museumMdAutocompleteCtrl',
    controllerAs: '$museummdAutocompleteCtrl',
    scope:        {
    ...
    template:     function (element, attr) {
      ...
      return '\
        <md-autocomplete-wrap\
            layout="row"\
            ng-class="{ \'md-whiteframe-z1\': !floatingLabel, \'md-menu-showing\': !$museummdAutocompleteCtrl.hidden }">\
          ' + getInputElement() + '\
          <md-progress-linear\
              class="' + (attr.mdFloatingLabel ? 'md-inline' : '') + '"\
              ng-if="$museummdAutocompleteCtrl.loadingIsVisible()"\
              md-mode="indeterminate"></md-progress-linear>\
          <md-virtual-repeat-container\
              md-auto-shrink\
              md-auto-shrink-min="1"\
              ng-mouseenter="$museummdAutocompleteCtrl.listEnter()"\
              ng-mouseleave="$museummdAutocompleteCtrl.listLeave()"\
              ng-mouseup="$museummdAutocompleteCtrl.mouseUp()"\
              ng-hide="$museummdAutocompleteCtrl.hidden"\
              class="md-autocomplete-suggestions-container md-whiteframe-z1"\
              ng-class="{ \'md-not-found\': $museummdAutocompleteCtrl.notFoundVisible() }"\
              role="presentation">\
            <ul class="md-autocomplete-suggestions"\
                ng-class="::menuClass"\
                id="ul-{{$museummdAutocompleteCtrl.id}}">\
              <li md-virtual-repeat="item in $museummdAutocompleteCtrl.matches"\
                  ng-class="{ selected: $index === $museummdAutocompleteCtrl.index }"\
                  ng-click="$museummdAutocompleteCtrl.select($index)"\
                  md-extra-name="$museummdAutocompleteCtrl.itemName">\
                  ' + itemTemplate + '\
                  </li>' + noItemsTemplate + '\
            </ul>\
          </md-virtual-repeat-container>\
        ...
        <aria-status\
            ...
            role="status"\
            aria-live="assertive">\
          <p ng-repeat="message in $museummdAutocompleteCtrl.messages track by $index" ng-if="message">{{message}}</p>\
        </aria-status>';
      ...
      function getNoItemsTemplate() {
        ...
        return template
            ? '<li ng-if="$museummdAutocompleteCtrl.notFoundVisible()"\
        ...
        if (attr.mdFloatingLabel) {
          return '\
            <md-input-container flex ng-if="floatingLabel">\
              ...
              <input type="search"\
                  ...
                  id="{{ inputId || \'fl-input-\' + $museummdAutocompleteCtrl.id }}"\
                  name="{{inputName}}"\
                  autocomplete="off"\
                  ng-required="$museummdAutocompleteCtrl.isRequired"\
                  ng-readonly="$museummdAutocompleteCtrl.isReadonly"\
                  ng-minlength="inputMinlength"\
                  ng-maxlength="inputMaxlength"\
                  ng-disabled="$museummdAutocompleteCtrl.isDisabled"\
                  ng-model="$museummdAutocompleteCtrl.scope.searchText"\
                  ng-keydown="$museummdAutocompleteCtrl.keydown($event)"\
                  ng-blur="$museummdAutocompleteCtrl.blur()"\
                  ' + (attr.mdNoAsterisk != null ? 'md-no-asterisk="' + attr.mdNoAsterisk + '"' : '') + '\
                  ng-focus="$museummdAutocompleteCtrl.focus()"\
                  aria-owns="ul-{{$museummdAutocompleteCtrl.id}}"\
                  ...
                  aria-expanded="{{!$museummdAutocompleteCtrl.hidden}}"/>\
          ...
          return '\
            <input flex type="search"\
                ' + (tabindex != null ? 'tabindex="' + tabindex + '"' : '') + '\
                id="{{ inputId || \'input-\' + $museummdAutocompleteCtrl.id }}"\
                name="{{inputName}}"\
                ng-if="!floatingLabel"\
                autocomplete="off"\
                ng-required="$museummdAutocompleteCtrl.isRequired"\
            ...
            <button\
                type="button"\
                tabindex="-1"\
                ng-if="$museummdAutocompleteCtrl.scope.searchText && !$museummdAutocompleteCtrl.isDisabled"\
                ng-click="$museummdAutocompleteCtrl.clear()">\
...
angular
  .module('material.components.autocomplete')
  .directive('museummdAutocompleteParentScope', museumMdAutocompleteItemScopeDirective);

function museumMdAutocompleteItemScopeDirective($compile, $mdUtil) {
  return {
    restrict: 'AE',
    compile: compile,
    terminal: true,
  ...
  function compile(tElement, tAttr, transclude) {
    return function postLink(scope, element, attr) {
      var ctrl = scope.$museummdAutocompleteCtrl;
...
museumMdAutocompleteItemScopeDirective.$inject = ["$compile", "$mdUtil"];
angular
    .module('material.components.autocomplete')
