        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                var dragging = function(e) {
                    ...
                    switch(dragDir) {
                        case 'top':
                            var height =  h + (offset * vy) ;
                            if ( height < rMinHeight ) { return; } 
                            if(scope.rFlex) { element[0].style.flexBasis = height + 'px'; }
                            else {            element[0].style.height = height + 'px'; }          
                            break;
                        case 'right':
                            var width = w - (offset * vx);
                            if ( width < scope.rMinWidth ) { return; }
                            if(scope.rFlex) { element[0].style.flexBasis = width + 'px'; }
                            else {            element[0].style.width = width + 'px'; }
                            break;
                        case 'bottom':
                            var height =  h - (offset * vy) ;
                            if ( height < rMinHeight ) { return; } 
